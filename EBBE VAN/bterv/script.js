...
function enterDungeon(dungeonId) {
    ...
    if ((currentDungeon.type === "Kis" || currentDungeon.type === "Nagy") && !currentDungeon.leaderCard) {
...
function startBattle() {
    ...
    const playerCardsData = selectedCards.map(cardId => playerCollection.find(card => card.id === cardId));
...
function toggleGMCard(cardId) {
    ...
    if (selectedCards.includes(cardId)) {
        ...
        // Ellenőrizzük, hogy nem lépi-e túl a maximális kártyaszámot
        if (selectedCards.length >= currentDungeon.maxCards) {
            alert(`Maximum ${currentDungeon.maxCards} kártyát választhatsz ki ehhez a kazamatához!`);
            return;
        }
        selectedCards.push(cardId);
        cardEl.classList.add('selected');
...
function saveDungeon() {
    ...
    if ((currentDungeon.type === "Kis" || currentDungeon.type === "Nagy") && !leaderCard) {
